  ...
  methods: {
    loadFavorites() {
      if (this.user) {
        ...
        this.$api.axios.get('favorite').then((response) => {
          this.favorites = response.data.favorite.map((favorite) => this.$store.getters.getProductById(
            favorite.product_id,
          ));
        });
      }
    ...
    login() {
      this.$api.axios.post('login', this.data).then((response) => {
    ...
    logout() {
      this.$api.axios.post('logout').then((response) => {
